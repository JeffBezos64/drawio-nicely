...
__author__ = 'Jake Miller (@LaconicWolf) ORIGINAL, EXTENDED BY jakay for people to have nice pictures in draw.io'
__date__ = '20221013'
__version__ = '0.02'
__description__ = """Parses the XML output from an nmap scan. The user
                  ...
                  already exists. This forms the first stage of creating a 
		information rich draw.io diagram
                  """
...
def flatten_csv(csv_name):
    """ingests the CSV file and then flattens and writes it back out"""
    rows = []
    empty_list = ['','','','','','','','','']
    with open(csv_name, mode='r') as csv_file:
        fieldnames = ["IP","Host","OS","Proto","Port","Service","Product","Service FP","NSE Script ID","NSE Script Output","Notes"]
        additional_fieldnames = fieldnames[3:]
    ...
    count = [0 for x in host_list]
    for i in range(0, len(ips)):
        for j in range(0, len(host_list)):
            if ips[i] == host_list[j]:
                count[j] += 1

    max_size = max(count)
    new_headings_list = fieldnames[0:3]
    for i in range(max_size):
        for j in range(len(additional_fieldnames)):
            new_headings_list.append(additional_fieldnames[j] + str(i))
    ...
    for host in host_list:
        for row in rows:
            if host == row[0]:
                holding_dict[host].append(row[3:])
                holding_dict[host].append([''])
                if proto_count == 0:
                    holding_dict[host].insert(0,row[1:3])
                proto_count += 1
        if proto_count < max_size:
            for i in range(0, (max_size - proto_count)):
                holding_dict[host].append(empty_list)

        proto_count = 0

    for host in host_list:
        flattened_list = []
        temp_list = holding_dict[host]
        temp_list_1 = [i for s in temp_list for i in s]
        holding_dict[host] = temp_list_1

    asdf_list = []
    for key,value in holding_dict.items():
        holding1 = value
        holding1.insert(0,key)
        asdf_list.append(holding1)

    with open(csv_name, 'w') as f:
        f.truncate()
        write = csv.writer(f)
        write.writerow(new_headings_list)
        write.writerows(asdf_list)
...
def main():
    ...
    for filename in args.filename:
        ...
        if args.flattened_csv:
            print("FLATTENING XML TO CSV")
            parse_to_csv(data)
            flatten_csv(csv_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-u", "--udp_open", 
                        help="Displays the UDP ports identified as "
                             "open|filtered",
                        action="store_true")
    parser.add_argument("-fl", "--flattened_csv", 
                        nargs='?', const='scan.csv',
                        help="Outputs a flattened csv for use in other tools")
